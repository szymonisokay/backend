...
const getOffers = asyncHandler(async (req, res) => {
	const { type, location, minPrice, maxPrice, minArea, maxArea, sort } = req.query
	...
	const offers = await Offer.find(query).where('is_published', true).sort(sortOptions)
...
const getOffer = asyncHandler(async (req, res) => {
	...
	const offer = await Offer.findById(offerId).populate({
		path: 'user',
		select: '-password -user_role_id -__v',
	})
...
const updateOffer = asyncHandler(async (req, res) => {
	...
	res.status(201).json({ msg: 'Offer updated successfully', offer: updatedOffer })
})
